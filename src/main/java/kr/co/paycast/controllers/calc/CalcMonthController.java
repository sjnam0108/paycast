 ...
 * 무인 주문 결제 시스템 월별매출
 */
@Controller("calc-month-controller")
@RequestMapping(value="/calc/statsMonth")
public class CalcMonthController {
	private static final Logger logger = LoggerFactory.getLogger(CalcMonthController.class);
    ...
    public String index(Model model, Locale locale, HttpSession session,
    	...
    	msgMgr.addViewMessages(model, locale,
    			new Message[] {
					new Message("pageTitle", "statsMonth.title"),
					new Message("title_termdate", "statsDay.termdate"),
					new Message("title_month", "statsMonth.month"),
					new Message("title_menu", "statsMonth.menu"),
					new Message("title_amount", "statsMonth.amount"),
					new Message("title_amt", "statsMonth.amt"),
					new Message("title_salesamt", "statsMonth.salesamt"),
					new Message("title_totalamt", "statsMonth.totalamt")
    			});
	...
	public @ResponseBody List<CalcStatsItem> read(@RequestBody Map<String, Object> model, Locale locale, HttpSession session) {
		...
		try {
			...
			resultList = calculateService.getCalcMonthRead(fromDate, toDate, storeId);
