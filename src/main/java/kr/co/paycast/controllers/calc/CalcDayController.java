 ...
 * 무인 주문 결제 시스템 일별매출
 */
@Controller("calc-day-controller")
@RequestMapping(value="/calc/statsDay")
public class CalcDayController {
	private static final Logger logger = LoggerFactory.getLogger(CalcDayController.class);
    ...
    public String index(Model model, Locale locale, HttpSession session,
    	...
    	msgMgr.addViewMessages(model, locale,
    			new Message[] {
					new Message("pageTitle", "statsDay.title"),
					new Message("title_termdate", "statsDay.termdate"),
					new Message("title_day", "statsDay.day"),
					new Message("title_menu", "statsDay.menu"),
					new Message("title_amount", "statsDay.amount"),
					new Message("title_amt", "statsDay.amt"),
					new Message("title_salesamt", "statsDay.salesamt"),
					new Message("title_totalamt", "statsDay.totalamt")
    			});
	...
	public @ResponseBody List<CalcStatsItem> read(@RequestBody Map<String, Object> model, Locale locale, HttpSession session) {
		...
		try {
			...
			resultList = calculateService.getCalcDayRead(fromDate, toDate, storeId);
