...
<c:if test="${menuTotCnt gt 1}">
				...
				<div id="layout-sidenav" class="layout-sidenav sidenav sidenav-vertical bg-sidenav-theme">
					...
					<div class="app-brand logo">
						<span class="app-brand-logo logo-frame">
							...
							<div class="navbar-user nav-item dropdown">
								...
								<div class="dropdown-menu dropdown-menu-right">
									<a href="javascript:void(0)" class="dropdown-item" id="pwd-update-btn">
										<i class="fas fa-key text-light"></i> &nbsp; ${navbar_passwordUpdate}</a>
									<div class="dropdown-divider"></div>
									<a href="javascript:void(0)" class="dropdown-item" id="logout-btn">
										<i class="fas fa-sign-out-alt text-danger"></i> &nbsp; ${navbar_logout}</a>
								</div>
							</div>
						</div>
					</nav>
...
<c:if test="${isStoreSwitcherMode && not empty sessionScope['loginUser'] && sessionScope['loginUser'].storeSwitcherShown}">

								<!--  Store switcher -->
								<div class="btn-group pl-2">
									<button class="btn btn-default btn-sm dropdown-toggle" type="button" data-toggle="dropdown">
										<span class="fas fa-map-marker-alt"></span>
										<span class="pl-1" id="curr-store-name">${sessionScope['loginUser'].dispStoreName}</span>
									</button>
									<div class="dropdown-menu scrollable-dropdown-menu">
									
<c:forEach var="item" items="${sessionScope['loginUser'].userStores}">
	<c:choose>
		<c:when test="${sessionScope['currentStoreId'] eq item.storeId}">

										<a class="dropdown-item active" href="javascript:changeStore('${item.storeId}','${item.storeName}')">
											<span class="fas fa-map-marker-alt"></span>
											<span class="pl-1">${item.storeName}</span>
										</a>

		</c:when>
		<c:otherwise>

										<a class="dropdown-item" href="javascript:changeStore('${item.storeId}','${item.storeName}')">
											<span class="fas fa-map-marker-alt"></span>
											<span class="pl-1">${item.storeName}</span>
										</a>
...
function changeStore(id, name) {
	...
	$("#curr-store-name").text(name);
	
	showWaitModal();
	location.href = "/changestore?storeId=" + id + "&uri=" + window.location.pathname;
}
