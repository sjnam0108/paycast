...
<!-- Content -->
		...
		<div id="error_area">
			...
			<div class="msg_box" style="width: 100%; left: 0; margin-left: 0;">
				<c:if test="${not empty smilepay}">
					<div style="font-size: 50px; margin-top: 55px;">${smilepay}</div>
				</c:if>
		...
		<div id="error_foot">
			<c:if test="${empty NOTFOUND}">
				<a href="javascript:window.history.back();"><span class="fas fa-arrow-left fa-2x"></span></a>
			</c:if>
			<c:if test="${not empty NOTFOUND && (NOTFOUND eq 'BASKET')}">
...
#error_area h3 { font-size:90px; color:#fb2b66; letter-spacing:-0.5px; margin-top:50px; margin-bottom:10px; font-weight:700;}
#error_area h4 { font-size:30px; color:#000; }
